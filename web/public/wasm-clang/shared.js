...
class API {
  ...
  hostLog(message) {
    ...
    // Use hostLogWrite if available, otherwise fallback to hostWrite
    if (this.hostLogWrite) {
        this.hostLogWrite(`${yellowArrow}${message}`);
    } else {
  ...
  async hostLogAsync(message, promise) {
    ...
    if (this.showTiming) {
      const green = '\x1b[92m';
      const normal = '\x1b[0m';
      const msg = ` ${green}(${msToSec(start, end)}s)${normal}\n`;
      if (this.hostLogWrite) {
          this.hostLogWrite(msg);
      } else {
          this.hostWrite(msg);
      }
    }
